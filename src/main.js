...
document.querySelector('#app').innerHTML = `
  <main class="pulse-container">
    ...
    <section class="search-section">
      <form id="search-form">
        <input id="search-input" type="text" placeholder="Search the web..." autocomplete="off" />
    ...
    <section class="dashboard">
      ...
      <article class="card links-card">
        ...
        <button class="add-link-button" id="add-link-button">+ Add Link</button>
      </article>
    </section>
  </main>
  ...
  <div class="settings-panel" id="settings-panel">
    <div class="settings-header">
      <h2>Settings</h2>
      <button type="button" class="close-settings" id="close-settings" aria-label="Close settings">×</button>
    </div>
...
function updateClock() {
  ...
  document.querySelector("#date").textContent = now.toLocaleDateString("en-IN", {
...
function addCustomLink(linkData, index) {
  ...
  link.innerHTML = `
    <a class="quick-link" href="${linkData.url}" target="_blank" rel="noopener noreferrer">
      <span class="link-name">${linkData.name}</span>
    </a>
    <button class="delete-link" data-index="${index}" aria-label="Delete link">×</button>
  `
...
document.querySelectorAll(".theme-option").forEach(button => {
  button.addEventListener("click", function () {
    ...
    localStorage.setItem("pulse-theme", button.dataset.theme)
  })
})
...
document.querySelectorAll(".accent-option").forEach(button => {
  button.addEventListener("click", function () {
    ...
    localStorage.setItem("pulse-accent", button.dataset.accent)
  })
})

document.body.dataset.theme = localStorage.getItem("pulse-theme") || "dark"
document.body.dataset.accent = localStorage.getItem("pulse-accent") || "blue"
...
async function getWeather(latitude, longitude) {
  ...
  try {
    ...
    document.querySelector("#weather-condition").textContent = getWeatherDescription(weatherCode)
    document.querySelector("#weather-icon").textContent = getWeatherIcon(weatherCode)
  } catch (error) {
    console.error("Weather error:", error)
    document.querySelector("#weather-condition").textContent = "Unable to load weather"
  }
}
...
function getUserLocation() {
  if (!navigator.geolocation) {
    document.querySelector("#weather-location").textContent = "Location is not supported"
    return
  }
  navigator.geolocation.getCurrentPosition(
    ...
    function () {
      document.querySelector("#weather-location").textContent = "Location permission denied"
      document.querySelector("#weather-condition").textContent = "Unable to get your location"
    }
  )
}
...
async function getLocationName(latitude, longitude) {
  ...
  try {
    ...
    document.querySelector("#weather-location").textContent = state ? `${city}, ${state}` : city
  } catch (error) {
    console.error("Location name error:", error)
    document.querySelector("#weather-location").textContent = "Location unavailable"
  }
}
